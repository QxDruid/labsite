...
# импорт библиотек
import numpy as np  # содержит модуль случайного выбора
import xlwt         # Для работы с документами Excel
import os as sys    # для работы файловой системы
...
# <<<----------------------------------     Настройки листа Excel    ---------------------------------->>>
    ...
    for i in array:
        ...
        if i in with_test_subject:
        	ws.write(int(i) - shift_vert, 1 + shift_hor,1, style_main) # Пишем 1 в колонку 'индекс испытания'
        	ws.write(int(i) - shift_vert, 3 + shift_hor, int(np.random.choice(table.data[1][int(rand_object)-1], 1, replace = False)), style_main) # случайно выбираем и записываем 'Тест предмет'
...
# <<<_______________________________________Вызов Функций _________________________________________>>>
if __name__ == "__main__":
    ...
    table_type = input()
    create_table(fname, table_type) # вызываем функцию создания таблицы
    print('Ok')
