...
def send_verification_email(app, destination, key):
    with app.app_context():
        ...
        msg.body = f'Для подтверждения отправки Отзыва в ИЛТССД перейдите по ссылке xn--d1ahjyae.xn--p1ai/confirm?uuid={key}'
        msg.html = f'<h3>Для подтверждения отправки Отзыва в ИЛТССД перейдите по ссылке</h3><p><a href="xn--d1ahjyae.xn--p1ai/confirm?uuid={key}">Подтвердить отправку</a></p>'
        try:
            mail.send(msg)
        except:
            pass
